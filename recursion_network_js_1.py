...
for epoch in range(5):
  ...
  for batch, (x, y_truth) in enumerate(dataloader):
    ...
    loop.set_description('epoch:{}, batch loss:{:.4f}, avg train loss:{:.3f}, train acc:{:.6f}'.format(epoch, loss, loss_report, acc_report))
    loop.update(1)
...
model2.features[0] = Conv2d(3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
model2.classifier[6] = Linear(in_features=4096, out_features=test_dataset.num_classes, bias=True)
